# LSTM-QoE code for LIVE Netflix Database
...
from find_TrV_index import find_TrV_index
...
# Dataset information
no_videos = 112
no_training_videos = 91
no_validation_videos = 20
...
for test_video_no in range(1,no_videos+1):
	...
	for ii in range(no_training_videos):
		...
		a, b = find_TrV_index(training_video_no)

		video_content = loadmat(path1 + "content_" + str(a)+"_seq_" + str(b) + ".mat")				

		STSQ = np.transpose(video_content['STRRED_vec_avg1sec_with_rebuf'])
		Nrebuffers = np.transpose(video_content['Nrebuffers_avg1sec'])  
		TSL = np.transpose(video_content['TSL_avg1sec'])
		...
		score_continuous = np.transpose(video_content['continuous_subj_score_avg1sec'])
		score_continuous_CIhigh = np.transpose(video_content['continuous_subj_score_CIhigh_avg1sec'])
		score_continuous_CIlow = np.transpose(video_content['continuous_subj_score_CIlow_avg1sec'])
	...
	for ii in range(no_validation_videos):
		...
		a, b = find_TrV_index(validation_video_no)
	...
	a, b = find_TrV_index(test_video_no)
